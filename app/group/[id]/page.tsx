...
export default function GroupStatsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 먼저 fast API를 시도
        let response = await fetch(`/api/group-stats-fast/${groupId}?month=${selectedMonth}`);

        // Fast API 실패시 fallback API 시도
        if (!response.ok) {
          console.log('Fast API failed, trying fallback API...');
          response = await fetch(`/api/group-stats/${groupId}?month=${selectedMonth}`);
        }

        if (!response.ok) throw new Error('Failed to fetch from both APIs');
