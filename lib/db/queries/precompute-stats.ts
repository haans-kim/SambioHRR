...
/**
 * 담당별 통계는 팀 통계에서 API 레벨에서 집계됨 (employees 테이블에 division_name 없음)
 * teams-fast API의 divisionsWithStats 로직 참조
 */
...
export function getPrecomputedTeamStats(month: string, centerCode?: string) {
  let query = `
    ...
    LEFT JOIN organization_master om ON om.org_name = mts.team_name
  ...
  if (centerCode) {
    query += ` AND (om.parent_org_code = ? OR mts.center_name = (SELECT org_name FROM organization_master WHERE org_code = ?))`;
    params.push(centerCode, centerCode);
  }

  query += ` ORDER BY mts.team_name`;

  const teamStats = db.prepare(query).all(...params);
  return teamStats;
}