...
def upload_eam_file(file_path, sheet_name, db_path):
    """Upload EAM data file to the database"""
    logger.info(f"\n파일 로드: {file_path}")
    ...
    try:
        rows_inserted = db_manager.insert_dataframe('eam_data', df_transformed, if_exists='append')
...
def main():
    ...
    sheet_name = 'EAM_refined'

    try:
        rows = upload_eam_file(file_path, sheet_name, db_path)
    except Exception as e:
        logger.error(f"업로드 실패: {e}")
        return
    ...
    for month in ['2025-08', '2025-09', '2025-10']:
        cursor.execute("SELECT COUNT(*) FROM eam_data WHERE strftime('%Y-%m', ATTEMPTDATE) = ?", (month,))
        count = cursor.fetchone()[0]
        logger.info(f"{month}: {count:,}건")

    # 전체 건수
    cursor.execute("SELECT COUNT(*) FROM eam_data")
